...
class ItemSearchMovieAdapter(private val context: Context, list: ArrayList<ModelMovie>) :
    RecyclerView.Adapter<ItemSearchMovieAdapter.CardViewHolder>() {

    var arrList: ArrayList<ModelMovie> = list

    var sizeSearchMovie = 0
    ...
    override fun onBindViewHolder(p0: CardViewHolder, p1: Int) {
        ...
        if (!arrList[p1].Title.isNullOrEmpty())p0.tvTitle.text = arrList[p1].Title
        if (!arrList[p1].ReleaseDate.isNullOrEmpty()) p0.tvYear.text = arrList[p1].ReleaseDate.substring(0, 4)
        else arrList[p1].ReleaseDate = ""
